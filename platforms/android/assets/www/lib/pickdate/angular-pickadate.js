  ...
  angular.module('pickadate', [])
    ...
    .directive('pickadate', ['$locale', '$sce', '$compile', '$document', '$window', 'pickadateUtils',
        ...
        '<div class="pickadate" ng-show="displayPicker" ng-style="styles">' +
          '<div class="pickadate-header">' +
            '<div class="pickadate-controls">' +
              ...
              '</a>' +
            '</div>'+
            '<h3 class="pickadate-centered-heading" style="margin-bottom:15px">' +
	  ...
	  doneDates: '='
	    ...
	    if(indexOf.call(selectedDates, date.date) >= 0) {
	      for(var idx=0; idx<date.classNames.length; idx++) {
		if(date.classNames[idx] === 'pickadate-done') {
		  date.classNames.splice(idx, 1);
		}
	      }
	    } else {
	      for(idx=0; idx<scope.doneDates.length; idx++) {
		if(date.date === scope.doneDates[idx]) {
		  //alert('18');
		  //date.classNames.concat('pickadate-done');
		  return date.classNames.concat('pickadate-done');
		}
	      }
	    }
	      ...
	      for(var k=0; k<scope.doneDates.length; k++) {
		if(scope.doneDates[k] == date) {
		  classNames.push('pickadate-done');
		}
	      }
